...
ggplot(simi30) +
  geom_col(aes(x = reorder(rgb_color, color_share2), y = color_share2),
           fill = simi30$color_hex) +
  scale_y_continuous(labels = scales::percent) +
  scale_x_discrete(labels = color_labels$color_label) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.background = element_rect(fill = NA),
  ...
  labs(title = "Simi Valley's zoning map doesn't match its response in TCRLUS",
    subtitle = paste0("Share of pixels of each color in ", CITY2, " excluding white"),
       x = "Color", y = "Pixel Share",
    caption = "Source: City of Simi Valley Zoning Districts map") 

ggsave("R/plots/FINAL_Simi_Valley.png")
```
